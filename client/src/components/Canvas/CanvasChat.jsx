...
const CanvasChat = ({ projectId, isOpen, onClose }) => {
    // Use the same state and functions as ProjectChat
    ...
    // Use the same theme styles as ProjectChat
    const themeStyles = {
    ...
    // Use the same useEffect for fetching messages and members
    useEffect(() => {
        if (!projectId) return;
        ...
        const loadMessages = async () => {
            const messagesRef = collection(db, 'projects', projectId, 'canvas-messages'); // Different collection for canvas chat
            const q = query(
                messagesRef,
    ...
    // Use the same scroll handler
    const handleScroll = async (e) => {
        const { scrollTop } = e.target;
        if (scrollTop === 0 && !isLoadingMore && hasMore) {
            setIsLoadingMore(true);
            const messagesRef = collection(db, 'projects', projectId, 'canvas-messages');
            const q = query(
                messagesRef,
                orderBy('timestamp', 'desc'),
                limit(50),
            ...
            mentions.forEach(async (mentionedEmail) => {
                if (mentionedEmail !== currentUser.email) {
                    await createNotification({
                        ...
                        type: 'mention',
                        message: `${currentUser.displayName || currentUser.email} mentioned you in canvas chat`,
                        projectId,
                        link: `/project/${projectId}/canvas`
                    });
                }
            });
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            setUploading(true);
            const storageRef = ref(storage, `projects/${projectId}/canvas-chat/${Date.now()}_${file.name}`);
    ...
    // Use the same JSX structure as ProjectChat
    return (
        <AnimatePresence>
            {isOpen && (
                <motion.div
                ...
                >
                    {/* Same JSX structure as ProjectChat */}
                    <motion.div
                    ...
                    >
                        <div>
                            <h3 className="font-semibold text-primary text-lg">Canvas Chat</h3>
                            <p className="text-sm text-muted-foreground">Canvas Communication</p>
                        </div>
                        <button
                            onClick={onClose}
